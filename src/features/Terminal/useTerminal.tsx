...
import { recordRun, setLatestPrompt, startCommand } from "./terminalHistorySlice";
...
export function useTerminal() {
  ...
  const promptStartMarkerRef = useRef<
    ReturnType<Terminal['registerMarker']> | null
  >(null);
    ...
    if (data.startsWith("A")) {
      ...
      promptFinalizeTimerRef.current = window.setTimeout(() => {
        if (collectingPromptRef.current) {
          ...
          console.log("Prompt finalized via timer:", { prompt });
          setCurrentPrompt(prompt);
          setLastUserPrompt(prompt);
          lastUserPromptRef.current = prompt;
          dispatch(setLatestPrompt(prompt));
          const parts = prompt.split("\n");
          promptLastLineRef.current = parts.length ? parts[parts.length - 1] : "";
      ...
      console.log("OSC 133;B - Command input start detected");
      ...
      try {
        ...
        console.log("Extracting prompt between markers:", { startLine, endLine });
        ...
        console.log("Prompt extracted via B marker:", { prompt });
        ...
        const prompt = (promptBufferRef.current || "").trimEnd();
        console.log("Prompt extracted via fallback buffer:", { prompt });
        ...
        if (prompt && prompt !== ">>") {
          ...
          console.warn("Ignoring invalid or empty fallback prompt:", { prompt });
        }
      } finally {
        promptStartMarkerRef.current = null;
      }
      ...
      console.log("OSC 133;C - Command execution start detected");
      try {
        ...
        const knownPromptLine = (lastUserPromptRef.current.split("\n").slice(-1)[0]) || "";
        if (lineText && !lineText.includes(knownPromptLine) && knownPromptLine) {
        ...
        if (lineIndex !== null && lineText) {
          ...
          if (knownPromptLine && lineText.includes(knownPromptLine)) {
            ...
            if (promptIndex !== -1) {
              cmd = lineText.slice(promptIndex + knownPromptLine.length).trim();
            } else {
              cmd = lineText.trim();
            }
          } else {
      ...
      console.log("OSC 133;D - Command finished detected");
      console.log({ collectingRef: collectingRef.current, commandInProgressRef: commandInProgressRef.current, pendingCaptureResolveRef: pendingCaptureResolveRef.current });
      if (
        ...
        try {
          ...
          const endLine = endMarker?.line ?? null;
          console.log({ startMarker: startMarkerRef.current, startLine, endMarker, endLine });
          ...
          if (startLine !== null && endLine !== null && endLine >= startLine) {
            ...
            for (let i = startLine + 1; i <= endLine; i++) {
              const line = buffer.getLine(i);
              console.log({ line, string: line?.translateToString(true) });
              if (line) {
                lines.push(line.translateToString(true));
              }
            }
          ...
          console.warn("Failed to extract output between markers (step D):", error);
          output = "";
        } finally {
          startMarkerRef.current = null;
        }
        ...
        console.log({ prompt: lastUserPromptRef.current, command: lastUserCommandRef.current, output });
        dispatch(
          recordRun({
            prompt: lastUserPromptRef.current,
            command: lastUserCommandRef.current,
            output,
            exitCode,
            startedAt: commandStartTimeRef.current || new Date().toISOString(),
